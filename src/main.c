...
// #endif // _DEBUG
	...
	if (!get_installed_python_version(py_version, BUFF_SIZE)) {
		fputws(L"Error: get_installed_python_version returned false.\n", stderr);
	}

#ifdef _DEBUG
	wprintf_s(L"%llu python releases have been deserialized.\n" 
			  L"Installed python version is %S\n", parse_results.parsed_struct_count, py_version);
#endif // _DEBUG
